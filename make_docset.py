...
# Old-style diagrams: display-math MathML tables full of arrow characters
# (compiled from itex \array{}), which render poorly. The rewrites/
# pipeline regenerates them as typst+fletcher SVGs named by a hash of the
# exact MathML markup; where a verified SVG exists, splice it in inline
# (inline SVG inherits Dash's dark-mode inversion, like tikzcd diagrams).
# The regex and arrow set must match rewrites/extract_mtables.py.
DIAGRAM_MATH_RE = re.compile(r'<math [^>]*display="block"[^>]*>.*?</math>',
                             re.S)
DIAGRAM_ARROWS = set("→⟶←⟵↠↪↣↦⇀⇌⇒⇐⇔⟹⟸⟺⤳⇢⇠↔⟷"
                     "↓↑⇓⇑↡↟↘↙↗↖⇘⇙⇗⇖")
SVG_PROLOG_RE = re.compile(r"^<\?xml[^>]*\?>\s*")


def replace_diagrams(text: str, diagrams: Path) -> tuple[str, int]:
    """Swap diagram mtables whose hash has a regenerated SVG. Runs on the
    raw mirror text, before any other rewriting, so hashes match the
    extraction pipeline's."""
    n = 0
    ...
    def sub(m: re.Match) -> str:
        ...
        if not svg_file.is_file():
            return xml
        n += 1
        svg = SVG_PROLOG_RE.sub("", svg_file.read_text(encoding="utf-8"))
        return ('<div class="typst-diagram" style="text-align: center; '
                'margin: 1em 0">' + svg + "</div>")

    return DIAGRAM_MATH_RE.sub(sub, text), n
...
def main() -> None:
    ...
    ap.add_argument("--diagrams", type=Path,
                    help="directory of <hash>.svg diagrams regenerated by "
                    "the rewrites/ pipeline, spliced in place of the "
                    "matching MathML mtables")
    args = ap.parse_args()
    ...
    for page_id, page_dir in iter_pages(args.html):
        ...
        if args.diagrams:
            text, n = replace_diagrams(text, args.diagrams)
            n_diagrams += n
    ...
    print(f"{written} pages written"
          + (f", {n_diagrams} diagrams regenerated" if args.diagrams else ""))
