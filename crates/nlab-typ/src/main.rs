...
type Cell = Vec<(&'static str, Val)>;

fn cell(kind: &str) -> Cell {
    vec![("k", Val::S(kind.to_string()))]
}
fn get<'a>(c: &'a Cell, key: &str) -> Option<&'a str> {
    c.iter().find(|(k, _)| *k == key).and_then(|(_, v)| match v {
        Val::S(s) => Some(s.as_str()),
        _ => None,
    })
}
fn set(c: &mut Cell, key: &'static str, v: String) {
    c.push((key, Val::S(v)));
}
fn kind(c: &Cell) -> &str {
    get(c, "k").unwrap()
...
fn grid_json(grid: &[Vec<Cell>]) -> String {
...
fn find_array(tex: &str) -> Option<(usize, usize, String)> {
    let b = tex.as_bytes();
...
fn split_depth0(text: &str, seps: &[&str]) -> Vec<String> {
    let b = text.as_bytes();
...
/// Port of read_group.
fn read_group(text: &str) -> (String, String) {
...
fn classify_cell(cell_src: &str) -> Cell {
...
fn absorb_spills(grid: &mut [Vec<Cell>]) {
    for r in 0..grid.len() {
...
fn merge_annotations(grid: &mut Vec<Vec<Cell>>) {
...
fn parse_formula(tex: &str) -> Result<String, String> {
...
fn main() {
    ...
    match mode {
        "dump" => {
            let node = mitex_parser::parse(&input, DEFAULT_SPEC.clone());
            print_tree(&node, 0);
        }
        "grid" => match parse_formula(&input) {
